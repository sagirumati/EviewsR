...
The `eviews` chunk automatically returns the outputs of each equation object as a dataframe, accessible via `chunkName$pageName_equationName`. For example, The $R^2$ of the `ols` equation object is `r EviewsR$eviewsr_page_ols$r2`, which can be accessed using `` `r knitr::inline_expr('EviewsR$viewsr_page_ols$r2')` ``. 

```{r}
EviewsR$ols$r2
EviewsR$eviewsr_page_ols$r2
EviewsR$eviewsr_page_ols$aic
EviewsR$eviewsr_page_olstable[7:9,1:5] |> as.matrix()
```
