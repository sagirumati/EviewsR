---
title: "Untitled"
output: bookdown::pdf_document2
    # keep_tex: true
header-includes:
  - \usepackage{subfig}
date: '2022-06-11'
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = F,prompt = T,fig.align = "center",fig.pos="h")
options(prompt = "EViews> ")
library(EviewsR)
```

## R Markdown

```{eviews mychunk,sagir="new",save_options="t=png,d=300",prompt=T,dev.args="axis(b) font(40,b)",eval=TRUE,fig.cap="somefigure",fig.subcap=c("A figure","Another figure")}
#| fig.show: hold
#| echo: TRUE
#| save_options: "t=png,-c"
#| fig.keep: new

wfcreate(wf=sagiru,page=mati) m 2000 2025
'open mychunk

genr y=@cumsum(nrnd)
genr x=@cumsum(nrnd)
genr z=@cumsum(nrnd)
'                         graph grap1.line z  
 '                            graph grap2.line y
   freeze(grap,mode=overwrite) x.line
equation ols.ls y c x

wfsave mychunk
```

```{r }
mychunk$ols

```

# R plots



```{r labe,fig.cap="another fig"}
print(knitr::opts_current$get("sagir"))
print(knitr::opts_current$get("fig.show"))
y=cumsum(rnorm(100))
x=cumsum(rnorm(100))

plot(x,y)
```

```{r eview-graph}
data=data.frame(y=runif(100),x=runif(100))
eviews_graph(data,save_path = "",frequency = "m",start_date = 1990,group = F,options = "m",graph_procs = "template eviews7")

```
